...
namespace ELDNET.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string username, string password, bool rememberMe)
        {
            ...
            var student = _db.StudentAccounts.FirstOrDefault(s => s.StudentId == username || s.Email == username);
            if (student != null)
            {
            ...
            var faculty = _db.FacultyAccounts.FirstOrDefault(f => f.FacultyId == username || f.Email == username);
            if (faculty != null)
            {
                string hashedPassword = HashPassword(password);
        ...
        public IActionResult SignUp(string fullName, string email, string password, string confirmPassword, string accountType)
        {
            ...
            bool emailExists = _db.StudentAccounts.Any(sa => sa.Email == email) ||
                               _db.FacultyAccounts.Any(fa => fa.Email == email);
            if (emailExists)
            {
            ...
            try
            {
                ...
                {
                    TempData["SignupError"] = "Invalid account type selected.";
                    TempData["ShowSignup"] = true;
                    return RedirectToAction("Login");
                }
                _db.SaveChanges();
            ...
            {
                TempData["SignupError"] = "An unexpected error occurred. Please try again.";
                TempData["ShowSignup"] = true;
                return RedirectToAction("Login");
            }
        }
